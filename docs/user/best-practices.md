...
nanograph supports OpenAI, Gemini, and LM Studio embedding providers. Set the provider in `nanograph.toml`:
...
[embedding]
provider = "lmstudio"
model = "nomic-embed-text-v1.5"   # required: must match the model loaded in LM Studio
```
...
# LM Studio: usually no key needed
# LMSTUDIO_API_KEY=
```

The default models are `text-embedding-3-small` (OpenAI, native 1536 dims) and `gemini-embedding-2-preview` (Gemini, native 3072 dims). Both support dimensionality reduction — nanograph passes your schema's `Vector(dim)` to the API automatically. LM Studio has no default model: you must set `model` to whatever you have loaded in LM Studio's Local Server tab, and the loaded model's native dimension must match your schema's `Vector(dim)`. Override the model with `model` in the config:
...
If no explicit provider is set, nanograph auto-detects: it picks Gemini when `GEMINI_API_KEY` is present and `OPENAI_API_KEY` is not, and defaults to OpenAI otherwise. LM Studio is never auto-detected — always select it explicitly.

OpenAI and LM Studio are text-only in nanograph today. Use Gemini when `@embed(...)` targets a `@media_uri(...)` field. See [embeddings.md](embeddings.md) for the current provider matrix and Gemini media limits.
